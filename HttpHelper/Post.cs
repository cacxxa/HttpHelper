...
namespace HttpHelper
{
    ...
    {
        /// <summary> Отправляет на сервер асинхронный запрос типа POST, добавляя к нему параметры </summary>
            ...
            HttpClientHandler handler = null)
        {
            ...
            using (HttpResponseMessage responseMessage =
                await MakeRequest(PostQueryMethodEnum.POSTAsync, url, action, content, headers, handler))
            {
                return await responseMessage.Content.ReadAsStringAsync().ConfigureAwait(false);
            }
        }

        /// <summary> Отправляет на сервер синхронный запрос типа POST, добавляя к нему параметры </summary>
        /// <param name="url"> URL сервера </param>
        /// <param name="action"> Идентификатор действия на сервере </param>
        /// <param name="parameters">
        ///     Массив параметров для передачи на сервер в формате <see cref="KeyValuePair{TKey,TValue}" />
            ...
            HttpClientHandler handler = null)
        {
            using (HttpContent content =
                new FormUrlEncodedContent(parameters ?? new KeyValuePair<string, string>[] { }))
            using (HttpResponseMessage responseMessage =
                MakeRequest(PostQueryMethodEnum.POST, url, action, content, headers, handler).Result)
            {
                return responseMessage.Content.ReadAsStringAsync().Result;
            }
        }

        private static async Task<HttpResponseMessage> MakeRequest(PostQueryMethodEnum postQueryMethod, string url,
            ...
            HttpClientHandler handler = null)
        {
            using (HttpClient client = handler != null ? new HttpClient(handler, true) : new HttpClient())
            {
                ...
                return postQueryMethod == PostQueryMethodEnum.POST
                    ? client.PostAsync(new Uri(url).AppendUri(action), content).Result
                    : await client.PostAsync(new Uri(url).AppendUri(action), content).ConfigureAwait(false);
            }
        }
    }
}